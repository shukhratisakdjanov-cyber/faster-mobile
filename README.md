...
The repository must first exist at the `repository` URL in `package.json`; then configure the protected `github-packages` environment and set `GITHUB_PACKAGES_PUBLISH_ENABLED=true` before the first release. The current Builder Bob build also emits a CommonJS export-map warning that should be resolved before publishing externally.
